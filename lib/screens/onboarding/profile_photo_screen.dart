...
class _ProfilePhotoScreenState extends State<ProfilePhotoScreen> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (!ImageUploadService.isValidImageFile(imageFile)) {
          setState(() {
            _error = 'Invalid image format. Please use JPG, PNG, or WebP.';
          });
          return;
        }
        ...
        if (!await ImageUploadService.isValidFileSize(imageFile)) {
          setState(() {
            _error = 'Image is too large. Maximum size is 10MB.';
          });
          return;
        }
  ...
  void _uploadImage() {
    ...
    try {
      ...
      // Store image locally without API calls
      onboardingProvider.updateProfilePhotoLocally(_selectedImage!);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Photo saved successfully! Final submission will happen at the review step.'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
            'You can add a profile photo later in your settings. Are you sure you want to skip this step?',
          ),
          actions: [
            TextButton(
