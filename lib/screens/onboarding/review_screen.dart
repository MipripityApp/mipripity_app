...
import '../../utils/user_update_util.dart';
...
class _ReviewScreenState extends State<ReviewScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _handleComplete() async {
    ...
    try {
      // First, get the user by email to ensure we have the correct PostgreSQL ID
      final userService = UserService();
      final userData = await userService.getUserByEmail(user.email);
      
      if (userData == null || userData['id'] == null) {
        throw Exception('Could not find user with email ${user.email}');
      }
      
      final userId = userData['id'];
      print('Found user with PostgreSQL ID: $userId');
      
      // Save this ID for future reference
      await userService.saveCurrentUserId(userId);
      
      // Prepare all user data collected during onboarding
      final completeUserData = {
        // Basic info (already in database)
        ...
        // Agency details (from step 3, if applicable)
        'agency_name': user.agencyName,
        'agency_verified': user.agencyVerified,
        
        // Onboarding metadata
        'terms_accepted_at': DateTime.now().toIso8601String(),
        'privacy_policy_accepted_at': DateTime.now().toIso8601String(),
        'email_verified': true,
        'phone_verified': true,
      ...
      await updateUserData(
        user.email,
        completeUserData,
        userId: userId, // Pass the PostgreSQL ID explicitly
      );
      
      // Upload profile photo if one was selected
      final prefs = await SharedPreferences.getInstance();
      final pendingProfileImagePath = prefs.getString('pending_profile_image');
      
      if (pendingProfileImagePath != null) {
        final imageFile = File(pendingProfileImagePath);
        if (imageFile.existsSync()) {
          // Upload the image using static method
