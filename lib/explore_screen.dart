...
class _ExplorePageState extends State<ExplorePage> with AutomaticKeepAliveClientMixin {
  ...
  // Fetch poll properties with improved error handling
Future<void> _fetchPollProperties() async {
  ...
  try {
    print('Fetching poll properties...');
    final properties = await PollPropertyApi.getPollProperties();
    print('Poll properties fetched successfully: ${properties.length} items');
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to load poll properties: ${e.toString()}'),
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => PollPropertyScreen(
          ...
          isUserLoggedIn: true, // User is logged in on explore screen
        ),
      ),
    );
  }
  ...
  // Enhanced Poll Properties Section with better error handling
...
// Enhanced Poll Property Card with better error handling for images
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(
                  height: 36,
                  child: property.suggestions.isEmpty
                      ? const Center(
                          child: Text(
                            'No suggestions yet',
                            style: TextStyle(
...
// Helper method to build property image with better error handling
...
// Enhanced Poll Properties Shimmer (same as before)
